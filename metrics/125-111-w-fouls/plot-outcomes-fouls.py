...
for fi in csvs:
    ...
    for i in range(len(df)):
        ...
        # scores_dict[alg]['WINS'] += run['Outcomes/wins/ppo-sa-x3']
        # + run['Outcomes/wins/ppo-cma'] + run['Outcomes/wins/ppo-sa'] + run['Outcomes/wins/ppo-dma']
        # scores_dict[alg]['DRAWS'] += run['Outcomes/draws/ppo-sa-x3']
        # + run['Outcomes/draws/ppo-dma'] + run['Outcomes/draws/ppo-sa'] + run['Outcomes/draws/ppo-cma']
        # scores_dict[alg]['LOSSES'] += run['Outcomes/losses/ppo-sa-x3']
        # + run['Outcomes/losses/ppo-sa'] + run['Outcomes/losses/ppo-cma'] + run['Outcomes/losses/ppo-dma']
        # scores_dict[alg]['ATTACK FOUL'] += run['Outcomes/atk_fouls/ppo-sa-x3']
        # + run['Outcomes/atk_fouls/ppo-cma'] + run['Outcomes/atk_fouls/ppo-dma'] + run['Outcomes/atk_fouls/ppo-sa']

        # scores_dict[alg]['WINS'] += run['Outcomes/wins/zero'] + run['Outcomes/wins/ou']
        # scores_dict[alg]['DRAWS'] += run['Outcomes/draws/zero'] + run['Outcomes/draws/ou']
        # scores_dict[alg]['LOSSES'] += run['Outcomes/losses/zero'] + run['Outcomes/losses/ou']
        # scores_dict[alg]['ATTACK FOUL'] += run['Outcomes/atk_fouls/zero'] + run['Outcomes/atk_fouls/ou']
